...
## Delegating a review to another agent

The Hunk daemon is local and shared, so an agent running in a different pane or worktree can review into the same live session by selecting it with `--repo`.

Send the request with the `herdr-agent-message` skill — it owns the reply protocol (reply-to pane, completion report, fallback collection). This section covers only what a Hunk review request must carry.

Put in the request:

- The session selector as an **absolute path** — `--repo /abs/path/to/repo`. A bare `.` resolves against the receiving agent's cwd, which is a different worktree.
- What to review — files, hunks, or concerns. Not just "review this".
- Where findings go: inline notes in the session via `hunk session comment apply --repo /abs/path/to/repo --stdin`, with `--author <name>` (or an `author` field per item) so the user can tell reviewers apart.
- What to send back: a short summary only — note count and top concerns. The notes themselves live in the session; don't have the full review text relayed over the message channel.

Read the result yourself rather than depending on the reply:

```bash
hunk session comment list --repo /abs/path/to/repo --json
```
