...
- `cd <dir> && grep/cat/find/rg/head/tail/ls <相対パス>` のように、`cd` で移動してから
  相対パスでファイル読み取り系コマンドを実行しない。区切りが `&&` か `;` か `||` か、
  1回の Bash 呼び出し内に複数コマンドを並べているか、は問わない
  （`cd X; grep ... a; echo ---; grep ... b` のような `;` 連結でも同じ問題が起きる）。
...
- `cd X && git add . && git commit -m "..."` のような、ファイル読み取りを伴わない
  一般的な連結（書き込み・git 操作等、区切りが `&&`/`;` いずれでも）は対象外で、
  引き続き許容する。
